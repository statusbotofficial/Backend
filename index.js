...
// In-memory shop storage: {guildId: {items: [], purchases: [], customApprovals: []}}
let shopData = {};

// Initialize shop data for a guild
function getShopData(guildId) {
    if (!shopData[guildId]) {
        shopData[guildId] = {
            items: [
                // Preset items - always available
...
// POST /api/shop/:guildId/items - Create custom item
app.post("/api/shop/:guildId/items", verifyDiscordToken, (req, res) => {
    const { guildId } = req.params;
    ...
    try {
        const shop = getShopData(guildId);
        const customItem = {
            id: `custom_${Date.now()}`,
            type: "custom",
            name,
        ...
        shop.items.push(customItem);
        shop.customApprovals.push({
...
// GET /api/shop/:guildId/items - Get shop items (public/approved only)
app.get("/api/shop/:guildId/items", (req, res) => {
    const { guildId } = req.params;
    
    try {
        const shop = getShopData(guildId);
        const publicItems = shop.items.filter(item => item.type === "preset" || item.status === "approved");
        
        res.json({ guildId, items: publicItems });
    } catch (err) {
        res.status(500).json({ error: "Failed to fetch items", details: err.message });
    }
});

// GET /api/shop/:guildId/items/all - Get all items including pending (admin only)
app.get("/api/shop/:guildId/items/all", verifyDiscordToken, (req, res) => {
    const { guildId } = req.params;
    
    try {
        const shop = getShopData(guildId);
        res.json({ guildId, items: shop.items });
    } catch (err) {
        res.status(500).json({ error: "Failed to fetch items", details: err.message });
    }
});

// POST /api/shop/:guildId/purchase - Purchase item
app.post("/api/shop/:guildId/purchase", verifyDiscordToken, (req, res) => {
    const { guildId } = req.params;
    ...
    try {
        ...
        if (item.status === "pending_approval") {
            return res.status(400).json({ error: "Item not approved yet" });
        }
...
// GET /api/shop/:guildId/purchases/:userId - Get user's purchases
app.get("/api/shop/:guildId/purchases/:userId", (req, res) => {
    const { guildId, userId } = req.params;
    
    try {
        const shop = getShopData(guildId);
        const userPurchases = shop.purchases.filter(p => p.user_id === userId);
        
        res.json({ userId, purchases: userPurchases });
    } catch (err) {
        res.status(500).json({ error: "Failed to fetch purchases", details: err.message });
    }
});

// POST /api/shop/:guildId/purchases/:purchaseId/redeem - Redeem item
app.post("/api/shop/:guildId/purchases/:purchaseId/redeem", (req, res) => {
    const { guildId, purchaseId } = req.params;
    const SECRET_KEY = process.env.BOT_STATS_SECRET || "status-bot-stats-secret-key";
    const authHeader = req.headers['authorization'] || '';
    ...
    try {
        const shop = getShopData(guildId);
        const purchase = shop.purchases.find(p => p.purchase_id === purchaseId);
        ...
        purchase.redeemed_at = new Date().toISOString();
        
        res.json({ success: true, purchase });
    } catch (err) {
        res.status(500).json({ error: "Failed to redeem purchase", details: err.message });
    }
});

// POST /api/shop/:guildId/approvals/:itemId/approve - Approve custom item
app.post("/api/shop/:guildId/approvals/:itemId/approve", verifyDiscordToken, (req, res) => {
    const { guildId, itemId } = req.params;
    
    try {
        const shop = getShopData(guildId);
        const item = shop.items.find(i => i.id === itemId);
        const approval = shop.customApprovals.find(a => a.item_id === itemId);
        
        if (!item || !approval) {
            return res.status(404).json({ error: "Item or approval not found" });
        }
        ...
        res.status(500).json({ error: "Failed to approve item", details: err.message });
    }
});

// POST /api/shop/:guildId/approvals/:itemId/reject - Reject custom item
app.post("/api/shop/:guildId/approvals/:itemId/reject", verifyDiscordToken, (req, res) => {
    const { guildId, itemId } = req.params;
    
    try {
        const shop = getShopData(guildId);
        const itemIndex = shop.items.findIndex(i => i.id === itemId);
        const approval = shop.customApprovals.find(a => a.item_id === itemId);
        
        if (itemIndex === -1 || !approval) {
            return res.status(404).json({ error: "Item or approval not found" });
        }
        ...
        approval.rejected_by = req.user?.id;
...
// GET /api/shop/:guildId/approvals - Get pending approvals
app.get("/api/shop/:guildId/approvals", verifyDiscordToken, (req, res) => {
    const { guildId } = req.params;
    
    try {
        const shop = getShopData(guildId);
        const pendingApprovals = shop.customApprovals.filter(a => a.status === "pending");
        const items = pendingApprovals.map(approval => {
            const item = shop.items.find(i => i.id === approval.item_id);
            return { ...item, approval };
        }).filter(item => item);
        
        res.json({ guildId, pending_items: items });
    } catch (err) {
